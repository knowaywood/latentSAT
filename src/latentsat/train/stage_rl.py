...
from latentsat.utils.verify import extract_conclusion, is_SAT, verify
...
class PolicyGradientTrainer:
    ...
    def _compute_reward(self, clauses: list[list[int]], satisfiable: bool, text: str) -> float:
        ...
        try:
            assignment = extract_conclusion(text)
        except Exception:
            assignment = {}
    ...
    def rollout(self, batch: Sequence[RLSample]) -> RolloutBatch:
        ...
        for _ in range(self.max_new_tokens):
            ...
            active_mask = (~finished).to(selected_log_probs.dtype)
            token_log_probs.append(selected_log_probs * active_mask)
            token_mask.append(active_mask)
            ...
            next_embeds = self.model._embed_tokens(next_token).to(dtype=self.model.model_dtype)
            current_embeds = torch.cat([current_embeds, next_embeds], dim=1)
            current_mask = torch.cat(
                [
                    current_mask,
                    torch.ones(batch_size, 1, dtype=current_mask.dtype, device=self.device),
                ],
                dim=1,
            )
    ...
    def train_epoch(self, dataloader: DataLoader[RLSample], epoch: int, log_every: int) -> None:
        self.model.train()
        for step, batch in enumerate(dataloader, start=1):
            ...
            if step % log_every == 0 or step == 1:
                ...
                print(f"sample_reward={rollout.rewards[0].item():.3f} sample_text={preview}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Policy-gradient RL training for LatentSAT")
